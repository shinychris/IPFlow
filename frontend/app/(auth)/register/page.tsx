...
export default function RegisterPage() {
  ...
  useEffect(() => {
    if (authError) {
      setError(typeof authError === 'string' ? authError : authError.message || '注册失败');
    }
  }, [authError]);
        ...
        <div className="w-full max-w-sm space-y-8">
          <div className="flex flex-col items-center gap-3">
            ...
            <div className="text-center">
              <h1 className="text-2xl font-bold" data-testid="text-register-title">注册</h1>
              <p className="text-sm text-muted-foreground mt-1">创建新账号以使用知识产权助手</p>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            {error && (
              <div className="p-3 rounded-md bg-destructive/10 text-destructive text-sm" data-testid="text-register-error">
                {typeof error === 'string' ? error : (error as Error)?.message || '注册失败'}
              </div>
            )}
          ...
          <div className="text-center text-sm text-muted-foreground">
            已有账号？
            <Link href="/login" className="text-primary ml-1 hover:underline" data-testid="link-login">
              返回登录
            </Link>
          </div>
        </div>
