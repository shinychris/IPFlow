...
**IPFlow 知识产权助手**是一款面向中国知识产权申报材料准备场景的专业工具应用，帮助用户高效生成、整理、核对与导出软件著作权（软著）、专利、商标三类申报材料。产品定位为材料辅助工具，不是完整的申请管理平台、代理业务系统或强 SaaS 商业平台。
...
| **流程标准化** | 统一材料准备流程：创建、填写、校验、导出 |
| **配套能力** | 当前代码保留组织、成员、角色与订阅配额管理，可按部署需要启用 |
...
- 用户认证（JWT 访问令牌 + 刷新令牌）
- 组织与成员管理（邀请、角色、加入组织，可选启用）
- 订阅与配额接口（计划、当前订阅、发票、用量、webhook，当前代码保留）
- 基础后台接口（仪表盘、用户/组织/计划、审计日志，当前代码保留）
...
| 订阅配额 | `GET /subscriptions/plans` `GET /subscriptions/current` |
| 基础后台 | `GET /admin/dashboard` `GET /admin/audit-logs` |
...
| 组织协作 | ✅ 已实现 | 可选配套能力：组织与成员管理、邀请加入 |
| 订阅配额 | ✅ 已实现 | 配套能力：计划/订阅/发票/用量/webhook |
| 基础后台 | ✅ 已实现 | 配套能力：统计、用户组织计划管理、审计日志 |
